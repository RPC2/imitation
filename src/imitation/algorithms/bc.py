...
import torch as th
import tqdm
from stable_baselines3.common import policies, utils, vec_env, preprocessing

from imitation.algorithms import base as algo_base
from imitation.data import rollout, types
from imitation.policies import base as policy_base
from imitation.util import logger as imit_logger
from imitation.util.util import identity
...
class BehaviorCloningLossCalculator:
    ...
    def __call__(
        ...
        gradient_parameters = list(
            filter(lambda p: p.grad is not None, policy.parameters())
        )
        stacked_gradient_norms = th.stack(
            [
                th.norm(p.grad.detach(), 2).to(policy.device)
                for p in gradient_parameters
            ]
        )
        gradient_norm = th.norm(stacked_gradient_norms, 2)
...
class BehaviorCloningTrainer:
    ...
    policy: policies.ActorCriticPolicy
    augmentation_fn: Optional[Callable[[th.tensor]]]
    ...
    def __call__(self, batch) -> BCTrainingMetrics:
        ...
        if self.augmentation_fn is not None:
            ...
            #         obs = obs * 255.0
            # TODO: Check obs scale. Do we need to rescale it to 255.0?
            obs = self.augmentation_fn(obs)
